...
class Machine extends Component {
    constructor(props) {
        ...
        oldCheck = this.props.available === 't'
            ? false
            : true
    ...
    clearLogs = () => {
        this.setState({currlogs: '', logs: '', slogs: ''})
        return this.state.id
    }

    xclose = () => {
        this.setState({showCheckIn: false, showCheckOut: false, checkedOut: false});
    }
    ...
    closePopup(outlog)
    {
        ...
        this.setState({showCheckIn: false, showCheckOut: false});
    ...
    togglePopup = (name, outlog) => {
        ...
        if (!this.state.checkedOut) {
            ...
            this
                .props
                .firebase
                .checkinMachine(this.props.id, this.props.name, log, id)
                .then(
                //log here as well
                this.childUpdate("\u00a0"))
        }
    ...
    handleSLogChange = (event) => {
        this.setState({slogs: event.target.value})
    ...
    closeLogs = (isSlog) => {
        if (isSlog) {
            this
                .props
                .firebase
                .recordSLog(this.props.id, this.state.slogs)
        }
    ...
    render()
    {
        return (
            <tr className="Machine" key={this.props.key}>
                ...
                <td className="KeyBox">
                    <ChangingButton
                        handleLogs={this.handleSubmit}
                        change={update => this.childUpdate = update}
                        green={!this.state.checkedOut}
                        id={this.props.id}
                        triggerParentUpdate={this.togglePopup}
                        name={this.state.user}></ChangingButton>
            ...
            <div className='Login'>
                <div className='PopInner'>
                    ...
                    <MiniStudentTable
                        ...
                        machineName={this.props.name}
                        machineLogs={this.props.logs}
                        closeFlip={this.props.closePopup}
                        change={this.props.myCallBack}></MiniStudentTable>
