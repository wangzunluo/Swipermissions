...
class MachineTable extends Component {
    ...
    clearLogs = () => {
      let machines = []
      this.state.clearFuncs.forEach((func) => {
        machines.push(func())
      })
      console.log(machines)
      machines.forEach((id) => {
        this.props.firebase.clearLogs(id)
      })
    }
    ...
    renderMachines() {
      ...
      return this.state.data.map((machine, i) => {
        return <MyMachine key={i} name={machine.name} id={machine.id} clear={update => this.state.clearFuncs.push(update)} user={machine.user} available={machine.available} logs={machine.logs} slogs={machine.slogs}/>
      })
    }
